...
Estsoft에서 주관하는 백엔드 오르미 교육과정에서 만난 비전공자와 전공자들로 이루어져 있습니다. "교육 과정이 끝나더라도 모두가 같은 목표를 가지고 같은 길을 걸어 가고 있다는 이야기를 드리고싶다"라는 목표를 가지고 해당 프로젝트를 시작하게 되었습니다. 잘부탁드립니다 :)
...
-   Follow / Unfollow 기능
    ...
    팔로잉한 유저와 채팅방을 만들어 1:1 채팅 서비스를 이용하실 수 있습니다.
    ```
    [# 팔로우 시연 영상](#57-팔로우-기능)
-   1대1 채팅
    ```
...
-   게시글 CRUD와 좋아요
    ```
    자신들의 일상을 공유하거나 지식을 나눌 수 있는 장소와 기능을 제공하고 있습니다.
    해당 기능은 제목과, 컨텐츠 , 다중 이미지 업로드를 제공하고 있습니다.
    
    마음에 드는 글에 좋아요를 남길 수도 있습니다.
    ```
    [# 게시물 시연 영상](#54-게시물-기능)<br>
    [# 좋아요 시연 영상](#56-좋아요-기능)
-   댓글과 대댓글 CRD
    ```
    댓글을 이용하여 게시글에 대한 의견을 남길 수도 있으며 댓글의 대댓글 기능 까지 지원하고 있습니다.
    댓글을 삭제하는 기능까지 제공 되고 있습니다.
    ```
    [# 댓글 시연 영상](#55-댓글-기능)
-   스터디 모임 CRUD
    ```
...
-   스터디 리스트 페이지네이션
    ```
    스터디 리스트의 경우 1페이당 12개의 게시물을 볼 수 있는 페이지네이션 기능을 제공하고 있습니다.
    ```
    [# 스터디 시연 영상](#58-스터디-기능)
-   게시글, 사용자, 스터디 검색 기능
    ```
    원하시는 키워드 검색을 통해서 
    게시물의 제목 또는 내용, 유저의 닉네임, 스터디 제목 또는 내용에 
    해당 키워드가 포함된 게시물,프로필,스터디들을 찾아 보실 수 있습니다.
    ```
    [# 검색 시연 영상](#59-검색-기능)
-   실시간 알림 기능
    ```
    ...
    ```
    [# Django Signal 프로세스](#344-실시간-알림-기능-django-signal-process)<br>
    [# 실시간 알림 시연 연상](#52-알림)
...
#### Asgi를 Nginx로 배포하기
```
Nginx가 WS 통신을 Uvicorn으로 전달해주는 과정에서 에러가 발생하였습니다.​
이를 해결하기 위해​ ​Nginx config를 WS 통신을 받을 수 있도록 설정하였고 Nginx로 들어오는 HTTP 통신을 Gunicorn으로 연결시켜 주었습니다.​

그 후 Gunicorn worker에 Uvicorn을 등록시켜 WS 통신을 Uvicorn으로 처리할 수 있도록 설정하였습니다.​

그 결과 단일 프로세스로 작동하는 Uvicorn의 단점을 극복하고, ASGI를 Nginx로 안정적으로 배포하는데 성공하였습니다. ​
```
#### 다중 이미지 업로드 기능
```
AWS S3에 저장된 URL을 받아 오는 것에서 문제점이 있었고, PostModel의 설계에서도 다중 이미지 업로드를 처리하는 데 문제가 ​있었습니다.​

PostModel의 설계 구조를 변경 후 PostImage 라는 새로운 테이블을 
생성하여 연결하였고, *AWS S3 이미지 업로드 모듈을 새로 작성하여 ​
저장된 URL을 받아 올 수 있도록 하였습니다.
```
**AWS S3 이미지 업로드 모듈 : uploads.py**

![스크린샷 2023-09-04 151954](https://github.com/ESTsoft-OrGo/OrGo/assets/107661525/e4535e3a-642d-4cfb-85d3-9aab18231013)
